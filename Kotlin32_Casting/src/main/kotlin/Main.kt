fun main(){
    ...
    // 자식 클래스에 있는 메서드 중 overriding 한 것이 아닌 것은 호출할 수 없다.
    // obj3.subMethod1()
    // obj4.subMethod2()
    ...
    // 하지만 SuperClass1 을 가지고 만든 객체에는 SubClass1 부분이 없기 때문에
    // 실행 중에 오류가 발생한다.
    // ClassCastingException
    val temp1: SuperClass1 = SuperClass1()
    // temp1 as SubClass1

    // 상속 관계나 인터페이스 구현 관계가 아닌 객체의 형 변환
    // 저녀 관계가 없다고 하더라도 문법적으로 오류가 발생하지는 않는다.
    // 실행 중에 오류가 발생한다.
    // ClassCastingException
    ...
    // 객체를 담을 변수에 타입을 설정하면 형 변환이 자동으로 이루어진다.
     val temp3: SuperClass1 = obj5

    // 객체를 담을 변수에 타입을 설정하지 않으면 변수의 타입은 객체의 클래스 타입으로 결정된다.
    // 이럴 때는 as 를 이용해 형 변환 해주면 형 변환된 클래스 타입 변수로 정의 된다.
    val temp4 = obj5 as SuperClass1
    println("temp3 : $temp3")
    println("temp4 : $temp4")

    // 참조 변수를 통해 접근할 수 있는 객체에 해당 클래스 영역이 있는지 검사
    ...
    // if 문 밖으로 나가면 형 변환 되지 않은 상태가 된다.
    // super3.subMethod1()
