fun main(){
    ...
    // 두 번째 매개 변수에 true 를 넣어주면
    // 모두 소문자로 변홚나 다음 코드값을 빼는 작업을 수행한다
    // 대소문자를 구분하고 같은가를 확인한다.
    println(str3.compareTo(str4, true))

    // 대소문자를 무시하고 같으지를 비교한다.
    if(str3.equals(str4, true)){
        println("대소문자를 무시하고 같습니다")
    }
    ...
    // String 은 문자열을 합치는 작업을 수행할 때 마다 새로운 문자열 객체가 계속 생성된다.
    // 이럴 때는 StringBuffer 를 사용한다.
