...
// protected : 상속 관계에서만 사용이 가능하다. 패키지, 모듈이 달라도 사용가능하다.
// internal : 모듈이 다르면 사용이 불가능하다.

// 생략시 public 으로 설정되고 internal 은 같은 모듈이라면 public 과 동일하다.

fun main(){
    // 같은 파일에 있는 클래스들은 아무 제약없이 사용이 가능하다
...
// 클래스에는 protected 설정 불가
//  protected class ProtectedClass1
internal class InternalClass1

open class SuperClass1{
    // kotlin 에서 자바로 변경될 때 모든 벰버 변수가 전부 private 변수로 변환된다.
    // kotlin 에서 접근 제한자를 설정하면 Java 로 변환될 때 setter//getter 생성에 대한 설정이 된다.
    private var a1 = 100
    public var a2 = 200
...
// 같은 모듈, 같은 파키지, 같은 파일에 있는 클래스를 상속하는 경우
// private 요소 사용 불가
