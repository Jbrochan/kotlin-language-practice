fun main(){
    ...
    // 매개변수를 가지고 있는 함수 호출
    test2(100, 11.11)
    // 매개변수를을 부족하게 전달할 경우에는 오류가 발생
    // test2(100)
    // 매개변수를을 더 많게 전달할 경우에도 오류가 발생
    // test2(100, 200, 300)

    // 함수를 호출할 때 넘겨주는 값을 어떤 매개변수에 담을지 정할 수 있다.
    test2(a2 = 11.11, a1 = 100)
...
// 기본 함수
// fun 함수이름(매개변수) : 반환타입
// Java 코드로 변경되면 파일이름클래스 내부의 static 메서드로 변경된다.
fun test1(){
    println("test1 호출")
    // 함수 밖에서 선언된 변수 사용
    println("a1 : $a1")
...
// 디컴파일러로 보면 test9는 다른 클래스 안에 들어가있다 -> RMI 라는 기술을 사용
fun test8(){
    println("test8 호출")
    println("-------------------------------")
